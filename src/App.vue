...
function onDragEnd(e: PointerEvent) {
  const dx = e.clientX - _x0
  const dy = e.clientY - _y0
  if (Math.abs(dx) < MIN_SWIPE && Math.abs(dy) < MIN_SWIPE) return
  if (Math.abs(dx) > Math.abs(dy)) {
    app().snakeDirection = dx > 0 ? 'right' : 'left'
  } else {
    app().snakeDirection = dy > 0 ? 'down' : 'up'
  }
}
...
<template>
  <div class="relative w-screen h-screen overflow-hidden bg-gray-950">
    ...
    <header class="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-5 py-3
      ...
      <span class="text-white/40 text-xs tracking-[0.3em] uppercase">Snake</span>
      <span class="text-white/90 font-mono text-lg tracking-widest">{{ application.points }} pts</span>
      <button
        v-if="!application.end"
    ...
    <Transition name="fade">
      <div
        v-if="application.paused && !application.end"
      ...
      >
        <div class="flex flex-col items-center gap-4 px-10 py-8 rounded-3xl
          ...
          <h2 class="text-white text-2xl font-light tracking-widest select-none">Paused</h2>
          <p class="text-white/40 font-mono text-2xl font-light">
          ...
          <button
            class="w-full py-3 rounded-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700
                   text-white font-medium tracking-wide transition-colors"
            @click="app().pause(); focusCanvas()"
          >▶ Resume</button>
          <button
            class="w-full py-3 rounded-full border border-white/20 bg-white/5 hover:bg-white/10
    ...
    <Transition name="fade">
      ...
      >
        <div class="flex flex-col items-center gap-5 px-10 py-8 rounded-3xl
          ...
          <p v-if="gameStarted" class="text-white/50 font-mono text-4xl font-light">
            {{ application.points }}<span class="text-lg ml-1">pts</span>
          </p>
          <!-- Speed slider — accessible here on mobile too -->
          <div class="w-full flex flex-col gap-1">
            <div class="flex justify-between text-xs text-gray-500">
              <span>Speed</span>
              <span class="font-mono text-gray-300">{{ application.speed }}x</span>
    ...
    <div
      class="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 select-none"
      style="display:grid; grid-template-columns:repeat(3,5rem); grid-template-rows:repeat(3,5rem); gap:6px;"
      @pointerdown.stop
    >
      <div />
      <button class="dpad-btn rounded-t-2xl" @click="dir('up')">
        <Icon>arrow_upward</Icon>
      </button>
      <div />
      <button class="dpad-btn rounded-l-2xl" @click="dir('left')">
        <Icon>arrow_back</Icon>
      </button>
      <div class="rounded-xl backdrop-blur-md bg-white/5 border border-white/10" />
      <button class="dpad-btn rounded-r-2xl" @click="dir('right')">
        <Icon>arrow_forward</Icon>
      </button>
      <div />
      <button class="dpad-btn rounded-b-2xl" @click="dir('down')">
